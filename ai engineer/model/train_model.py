...
from custom_callback import StopAtAuc
from custom_layer import RiskCalibrationLayer
from custom_loss import FocalBinaryCrossentropy


ROOT = Path(__file__).resolve().parents[2]
DATA_PATH = ROOT / "datascientist" / "final_dataset.csv"
MODEL_DIR = Path(__file__).resolve().parent / "saved_model"
MODEL_PATH = MODEL_DIR / "healpoint_no_show_model.keras"
PREPROCESSOR_PATH = MODEL_DIR / "preprocessor.pkl"
REPORT_PATH = Path(__file__).resolve().parent / "training_metrics.txt"
...
def build_model(input_dim: int) -> tf.keras.Model:
    ...
    model.compile(
        ...
        loss=FocalBinaryCrossentropy(gamma=2.0, alpha=0.35),
